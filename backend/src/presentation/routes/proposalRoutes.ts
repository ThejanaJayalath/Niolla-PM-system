...
router.post(
  ...
  [
    ...
    body('paymentPlan').optional().isIn(['FULL_PAYMENT', 'THREE_MONTH', 'SIX_MONTH']),
    body('installmentMonths').optional().isInt({ min: 1 }),
    body('monthlyInstallment').optional().isNumeric(),
...
router.patch(
  ...
  [
    ...
    body('validUntil').optional().trim(),
    body('notes').optional().trim(),
    body('status').optional().isIn(['DRAFT', 'SENT', 'CONFIRMED', 'LOST']),
  ],
  validate,
  updateProposal
);
